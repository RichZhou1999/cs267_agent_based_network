...
int main() {
    ...
    json zipcode_index_dict;
    infile >> zipcode_index_dict;
    // for (json::iterator it = zipcode_index_dict.begin(); it != zipcode_index_dict.end(); ++it) {
    ...
    for( int i = 0; i <simulation_agents_number; i++ ){
        ...
        for (int j = i + 1; j < simulation_agents_number; j++){
            int index1 = zipcode_index_dict[std::to_string(agents[i].zipcode)];
            int index2 = zipcode_index_dict[std::to_string(agents[j].zipcode)];
